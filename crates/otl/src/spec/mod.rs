...
use spec_compile::{BodyKind, CompiledOp, CompiledParam, CompiledSpec, ScalarKind};
...
///
/// The compiler validates every name and path it produces, but a cache
/// file is a separate trust boundary: it can be truncated, bit-flipped or
/// written by something else entirely. Since the request URL is
/// `base_url + op.path` concatenated as text, an operation path that
/// starts a new authority would send the bearer token to another host - so
/// the same rules are enforced again on the way in.
