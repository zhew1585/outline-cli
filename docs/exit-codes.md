...
- A document that compiles to more operations than the cache format allows makes `otl spec sync` fail (code 1, or 2 for a `--spec` file) rather than writing a cache that the next command would discard. The limits are deliberately far above any real API and are stated in the error.
