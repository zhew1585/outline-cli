...
| 3 | API request rejected | a 4xx that is not auth, not-found or exhausted rate limits | 4xx other than auth, not-found, and exhausted rate limits: validation error (400), a write the server refused because it was pinned to a revision the document has moved past (409 — `lastRevision`, sent for `--if-revision` and for the `revision` an exported file records; nothing was changed), a 429 that was not retried to exhaustion, a 4xx from an OAuth endpoint that is not 401/403/429 |
