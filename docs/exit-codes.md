...
- Credentials are bound to the instance that issued them. Pointing `OUTLINE_URL` at a different instance without switching profile exits **2** and sends nothing: a bearer token is only meaningful to its issuer, and forwarding it would hand a credential to a server that was never supposed to have it.
