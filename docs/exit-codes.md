...
| 3 | API request rejected | 4xx other than auth, not-found, and exhausted rate limits: validation error (400), a 429 that was not retried to exhaustion, a 4xx from an OAuth endpoint that is not 401/403/429, an `otl auth logout` whose server-side step (token revocation, or a `--purge` deletion) did not happen |
...
- `otl auth logout` exits **3** whenever a server-side step did not happen — a token that could not be revoked, or a `--purge` deletion the server refused. Two rules go with that code:
  ...
  - **If no retry could succeed** — the instance advertises no revocation endpoint, or the registration has no management token — the credentials are removed, because keeping them buys nothing. The exit code is still 3, since what was asked for did not happen.
