...
- Partial failure (code 9) means "what you got is real, and some of it is missing". Whatever succeeded is on stdout or on disk, every shortfall is described on stderr, and the exit code exists so that automation is never told a partial result is a complete one. Two situations produce it:
  ...
  - **A result that cannot be confirmed durable.** `otl docs export` flushes each directory it writes; if that flush fails, the files are readable now but are not known to survive a crash. The summary says so and `--json` reports `"durable": false`, because a backup script that only reads the exit code must not record such a run as clean.
