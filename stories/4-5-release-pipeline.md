...
- **体积门槛 4 MiB 的依据（R2-[4] 后修正）**：本分支实测 2_567_312 B（≈2.45 MiB，占 61%），但那是一条**只有发布配置、零功能代码**的分支——原来写的「约 40% 余量」是拿这个数说的，对合并后的产物不成立。R2 逐分支实测的增量：auth +317KB、commands +283KB、config +233KB、specsync +116KB；加性最坏估计合并后 darwin ≈3.35 MiB，musl 再高约 9% ≈**3.66 MiB ≈ 预算的 91%**。
  ...
  **Windows 实测到手后这个结论要收紧一次**：msvc = 3_757_568 B，是三个 x86_64 target 里**最小**的（msvc 3_757_568 < darwin 3_970_784 < musl 4_523_120，跨度 765_552 B）。所以「主导项是 x86_64 代码生成」说得过强了 —— 那是从 darwin-arm64 → musl-x86_64 这一条路径读出来的。把四个数放在一起看：**架构项 +16~21%，平台项 +9~20%，两者量级相当且相乘**，没有哪一项单独主导。值得注意的是 msvc 同样静态链接 CRT 却最小，所以分界不是「静态 vs 动态」，而是 ELF + `-Clink-self-contained=yes` 恰好是我们发布的组合里最贵的那个。两项都是固有成本，都不是能削掉的代码——这一点没变，per-target 预算的正当性也没变。`cargo bloat`（musl 构建）也证实无异常：`.text` 2.4 MiB 摊在 otl 414K / std 363K / aws-lc-sys 264K / rustls 249K / reqwest 162K / clap 124K，`rpassword` 与 `rustix` 连前 22 名都没进——auth 实现者「这两个很小、应先看静态 libc 与 LTO」的判断方向对，结论（是静态 libc）不对，真正的答案是架构。
  ...
  **结论：改成 per-target 预算，并把「回归门」与「NFR2 承诺」拆成两个数。** 一个数管四个 triple 时，它对最小的 target 毫无约束、对最大的 target 又误伤。现在：每个 target 的预算 = 该 target 实测（含 doctor 的 +33_040）× 1.08，**四个 target 全部实测、无临时值**，都落在 **91%**——门禁对每个 target 一样严：aarch64-darwin 3_750_000、msvc 4_094_000、x86_64-darwin 4_330_000、musl 4_920_000。另一条独立的 **NFR2 天花板 5_000_000 B** 对所有 target 硬失败，这样以后任何一次调预算都不可能悄悄授权突破承诺。警戒带从 85% 改为 **95%**（窄带「快撞了」而不是常亮告警），NFR2 侧另有 95% 的独立告警。
  **需要产品决策的一条（余量已写成明确数字，不埋在脚本逻辑里）**：musl 是最大产物，实测 4_523_120 B，NFR2 的 5_000_000 B 承诺**只剩 476_880 B（9.5%）余量**；doctor 落地后约剩 443_840 B。这就是「未来所有功能」在这条决定性 target 上的全部预算。脚本头部有一个专门的 HEADROOM 段把这三个数写死，免得后来人自己去推。musl 的预算 4_920_000 故意设在天花板以下，使任何一次预算编辑都无法悄悄授权突破承诺。若 musl 真要越过 ~4.9 MB，那是要和用户一起做的 NFR2 决定（抬高承诺或者削），不是这里的常量。
